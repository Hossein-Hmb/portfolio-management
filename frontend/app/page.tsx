...
export default function Dashboard() {
  ...
  if (error)
    return <div className="card neg">Failed to load: {String(error)}</div>;
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <MoneyKpi label="Net Deposits" amount={s.net_deposits ?? 0} />
        <MoneyKpi
          label="Total Return"
          amount={s.total_return ?? s.unrealized_pl}
          pct={s.total_return_pct ?? s.unrealized_pl_pct}
        />
        <MoneyKpi
          label="Day Change"
          amount={s.day_change}
        ...
        <div className="space-y-2">
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
            ...
            <div className="card">
              ...
              <div className={`kpi-value ${cls(perf.xirr)}`}>
                {fmtPct(perf.xirr)}
              </div>
            </div>
          </div>
          <div className="text-xs text-muted">
            Performance returns need dated cash flows and price history;
            imported snapshot positions show as 0.00% until more history is
            recorded.
          </div>
        </div>
      )}
